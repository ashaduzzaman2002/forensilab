...
interface Item { name: string; subtitle?: string; type: string }
  ...
  return (
    <section id="partners" className="bg-[#F5F7FA] px-[60px] py-[100px] max-md:px-6 max-md:py-[72px]">
      <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
        ...
        <h2 className="mb-9 font-heading text-[clamp(36px,5vw,66px)] font-[800] leading-none tracking-[-2px] text-primary">
          MOUs &amp; MOAs
        </h2>
        ...
        <div className="grid grid-cols-4 gap-[2px] max-md:grid-cols-2 max-sm:grid-cols-1">
          {filtered.map((item, i) => (
            <div key={i} className="flex min-h-[110px] items-center justify-center border border-border bg-white p-9">
