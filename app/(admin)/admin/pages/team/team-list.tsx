...
interface Item { _id: string; name: string; role: string; image: string; }
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {items.map((item) => (
            <div key={item._id} className={`${cardClass} p-5 text-center`}>
              {item.image && <div className="relative mx-auto size-20 overflow-hidden rounded-full ring-4 ring-blue-50"><Image src={item.image} alt={item.name} fill unoptimized className="object-cover" /></div>}
