...
import { createGalleryItem, updateGalleryItem, getGalleryUploadUrl } from "@/lib/actions/gallery";
...
async function uploadFile(file: File) { const { url, key } = await getGalleryUploadUrl(file.name, file.type); await fetch(url, { method: "PUT", body: file, headers: { "Content-Type": file.type } }); return { key }; }

export function GalleryForm({ item }: { item?: Data | null }) {
    ...
    <form action={handleSubmit} className="grid gap-6 lg:grid-cols-3 max-w-4xl">
      ...
      <div className={`${cardClass} p-6 space-y-3`}>
        ...
        <div onClick={() => fileRef.current?.click()} className="group relative flex aspect-video cursor-pointer items-center justify-center overflow-hidden rounded-xl border-2 border-dashed border-muted-foreground/20 bg-muted/30 transition hover:border-primary/40">
          {preview ? (<><Image src={preview} alt="Gallery" fill unoptimized className="object-cover" /><div className="absolute inset-0 flex items-center justify-center bg-black/0 transition group-hover:bg-black/30"><span className="rounded-lg bg-white/90 px-3 py-1.5 text-xs font-semibold opacity-0 shadow-lg backdrop-blur-sm transition group-hover:opacity-100">Replace</span></div></>) : (<div className="flex flex-col items-center gap-1 text-muted-foreground"><ImageIcon className="size-5" /><p className="text-xs">Upload</p></div>)}
        </div>
